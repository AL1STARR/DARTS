...
function initializeMonthDropdowns() {
  ...
  months.forEach(m => {
    ...
    if (currentYear === currentYear && parseInt(m.value) > currentMonth) {
      return;
    }
    const option = document.createElement('option');
  ...
  yearSelect.addEventListener('change', () => {
    ...
    months.forEach(m => {
      // Don't allow future months for current year
      if (selectedYear === currentYear && parseInt(m.value) > currentMonth) {
        return;
      }
      const option = document.createElement('option');
      option.value = m.value;
    ...
    if (selectedYear === currentYear) {
      monthSelect.value = String(currentMonth).padStart(2, '0');
    } else if (monthSelect.options.length > 1) {
      monthSelect.value = monthSelect.options[1].value;
    }
  });
}

const auditDownloadOverlay = document.getElementById('auditDownloadOverlay');
const downloadBtn = document.getElementById('downloadAuditBtn');
const downloadCloseBtn = document.getElementById('auditDownloadClose');
const downloadCancelBtn = document.getElementById('auditDownloadCancel');
const downloadSubmitBtn = document.getElementById('auditDownloadSubmit');

downloadBtn.addEventListener('click', () => {
  initializeMonthDropdowns();
  auditDownloadOverlay.classList.add('open');
});

downloadCloseBtn.addEventListener('click', () => auditDownloadOverlay.classList.remove('open'));
downloadCancelBtn.addEventListener('click', () => auditDownloadOverlay.classList.remove('open'));
auditDownloadOverlay.addEventListener('click', e => {
  if (e.target === auditDownloadOverlay) auditDownloadOverlay.classList.remove('open');
});
...
downloadSubmitBtn.addEventListener('click', async () => {
  ...
  downloadSubmitBtn.textContent = 'Downloading…';

  try {
    const response = await fetch(`/admin/audit-logs/download?year=${year}&month=${month}`);
    if (response.ok) {
      const blob = await response.blob();
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `audit-log-${year}-${month}.pdf`;
      document.body.appendChild(a);
      a.click();
      window.URL.revokeObjectURL(url);
      a.remove();
      auditDownloadOverlay.classList.remove('open');
      showToast('Audit log downloaded successfully.', 'success');
    } else {
      showToast('Failed to download audit log. Please try again.', 'error');
    }
  } catch (error) {
    console.error(error);
    showToast('Error downloading audit log. Please try again.', 'error');
  } finally {
    downloadSubmitBtn.disabled = false;
    downloadSubmitBtn.textContent = 'Download';
  }
});
